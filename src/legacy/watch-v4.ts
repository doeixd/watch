...
// Implementation
export function watch<
  T extends WatchTarget | HTMLElement | PreDefinedWatchContext<any, any, any>,
>(
  target: T,
  selectorOrGenerator?:
    | string
    | ((
      ...
      ) => Generator<ElementFn<any>, void, unknown>)
    | ((ctx: TypedGeneratorContext<any>) => AsyncGenerator<any, any, unknown>),
  generator?: (
    ctx: TypedGeneratorContext<any>,
  ) =>
    | Generator<ElementFn<any>, void, unknown>
  ...
  if (isPreDefinedWatchContext(target)) {
    ...
    const actualGenerator = selectorOrGenerator as (
      ctx: TypedGeneratorContext<any>,
    ) =>
      | Generator<ElementFn<any>, void, unknown>
      | AsyncGenerator<any, any, unknown>;
    ...
    // Check if this is an async generator (new pattern)
    const isAsyncGenerator = (fn: any) => {
      const result = fn({} as any);
      return result && typeof result[Symbol.asyncIterator] === "function";
    };

    // Only apply generator utilities to sync generators (old pattern)
    // For async generators (new pattern), skip utilities for now
    if (!isAsyncGenerator(actualGenerator)) {
      if (context.options.debounce) {
        wrappedGenerator = debounceGenerator(
          actualGenerator as any,
          context.options.debounce,
        );
      }
      ...
      if (context.options.throttle) {
        wrappedGenerator = throttleGenerator(
          actualGenerator as any,
          context.options.throttle,
        );
      }

      if (context.options.once) {
        wrappedGenerator = onceGenerator(actualGenerator as any);
      }
    }
  ...
  if (
    ...
    typeof selectorOrGenerator === "string"
  ) {
    const parent = target;
    ...
    controller.layer(delegatedGenerator as any);
    return controller;
  }
