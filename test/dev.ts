...
const cleanup2 = watch(
  ".dynamic-btn",
  createGenerator<HTMLButtonElement>(
    (ctx) =>
      function* () {
...
const debounceInputCtx = withDebounce(input("input.search"), 300);

const onceButtonCtx = once(
  withData(button("button.one-time"), { message: "I only work once!" }),
);
...
const testElements = [".enhanced-test", ".increment", ".search"];
