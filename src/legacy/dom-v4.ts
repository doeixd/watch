...
export function siblings<T extends HTMLElement = HTMLElement>(
  ...
  switch (detection.context) {
    ...
    case ApiContext.ASYNC_GENERATOR: {
      ...
      const [selector] = args;
      return ((element: HTMLElement) =>
        _impl_siblings(element, selector)) as ElementFn<HTMLElement, T[]>;
    }
...
export function createChildWatcher(...args: any[]): any {
  const detection = detectContext(args, createChildWatcher);
  ...
  switch (detection.context) {
    ...
    case ApiContext.SELECTOR: {
      ...
      // Fallback for direct mode without element
      return new Map();
    }

    case ApiContext.SYNC_GENERATOR: {
      // Sync generator mode: execute immediately
      const [selector, generator] = args;
      const currentContext = getCurrentContext();
      if (!currentContext) {
        throw new Error("No context available for createChildWatcher");
      }
    ...
    case ApiContext.ASYNC_GENERATOR: {
      ...
      }) as ElementFn<HTMLElement, Map<HTMLElement, any>>;
    }

    default: {
      // Unknown context, return empty map
      return new Map();
    }
  }
}
  ...
  const processChild = (child: Element) => {
    if (
      ...
      const runGenerator = async () => {
        ...
        const { registerParentContext } = await import("../core/context");
...
export function child(...args: any[]): any {
