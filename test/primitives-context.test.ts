...
test("getParentContext() - both global and context parameter work", async () => {
  ...
  await runOn(parent, function* (parentCtx) {
    ...
    const childMap = createChildWatcher(
      ".child-selector",
      function* (childCtx) {
...
test("createChildWatcher() - both global and context parameter work", async () => {
  ...
  await runOn(parent, function* (ctx) {
    ...
    const childMapCtx = createChildWatcher(
      ".child",
      function* (childCtx) {
...
test("Error handling - primitives throw errors outside context", () => {
  ...
  expect(() => self()).toThrow(
    "self() can only be called within a generator context",
  );
  expect(() => el("span")).toThrow(
    "el() can only be called within a generator context",
  );
  expect(() => el.all("span")).toThrow(
    "el.all() can only be called within a generator context",
  );
  expect(() => all("span")).toThrow(
    "all() can only be called within a generator context",
  );
  expect(() => cleanup(() => {})).toThrow(
    "cleanup() can only be called within a generator context",
  );
  expect(() => ctx()).toThrow(
    "ctx() can only be called within a generator context",
  );
  expect(() => getState("key")).toThrow(
    "State functions can only be called within a generator context",
  );
  expect(() => setState("key", "value")).toThrow(
    "State functions can only be called within a generator context",
  );
  expect(() => getParentContext()).toThrow(
