...
function normalizeTargetKey(
  target: WatchTarget<any>,
): string | WatchTarget<any> {
  ...
  if (target instanceof HTMLElement) {
    ...
    if (!elementToIdMap.has(target)) {
      elementToIdMap.set(
        target,
        `watch-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
      );
    }
  ...
  if (target instanceof NodeList) {
    ...
    const elementIds = Array.from(target)
      .map((el) => {
        if (el instanceof HTMLElement) {
          if (!elementToIdMap.has(el)) {
            elementToIdMap.set(
  ...
  if (
    ...
    target !== null &&
    "parent" in target &&
    "childSelector" in target
  ) {
    const delegationTarget = target as {
      parent: HTMLElement;
      childSelector: string;
    };
    if (!elementToIdMap.has(delegationTarget.parent)) {
      elementToIdMap.set(
        delegationTarget.parent,
        `watch-${Date.now()}-${Math.random().toString(36).substr(2, 9)}`,
      );
    }
...
function registerBehavior<El extends HTMLElement>(
  ...
  const elementMatchesSubject = (
    ...
    if (
      ...
      subj !== null &&
      "parent" in subj &&
      "childSelector" in subj
    ) {
      const delegationTarget = subj as {
        parent: HTMLElement;
        childSelector: string;
      };
      return (
        (delegationTarget.parent.contains(el) ||
          delegationTarget.parent === el) &&
    ...
    if (
      ...
      return (
        (scopedTarget.parent.contains(el) || scopedTarget.parent === el) &&
