...
watch(".dashboard", function* () {
  ...
  yield* on(userLoginEvent, (event, dashboard) => {
    ...
    console.log(
      `User ${event.detail.username} logged in at ${event.detail.timestamp}`,
    );
  ...
  yield* on(
    "notification:show",
    (
      event: CustomEvent<{ message: string; type: "info" | "error" }>,
      dashboard,
    ) => {
...
function isFormValid(form: HTMLFormElement): boolean {
  return Array.from(form.querySelectorAll("input, textarea")).every((field) =>
